'use client';
// T065 — Hero landing — bento grid identique prototype
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 md:grid-rows-[auto_auto] gap-4 sm:gap-5">
          ...
          <AnimatedSection className="md:col-span-3 md:row-span-2">
            <div className="rounded-bento relative h-full min-h-[340px] group cursor-pointer overflow-hidden">
              <img src={IMAGES.team} alt="" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          ...
          <AnimatedSection className="md:col-span-6">
            <div className="rounded-bento bg-pw-50 p-8 sm:p-10 flex flex-col justify-between min-h-[340px]">
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.5 }} className="mt-5 flex items-center gap-3">
                <div className="flex -space-x-2">
                  {['bg-pw-400', 'bg-pw-600', 'bg-sky-400', 'bg-pw-500'].map((bg, i) => (
                    <div key={i} className={`h-9 w-9 rounded-full ${bg} border-[2.5px] border-pw-50 flex items-center justify-center text-[10px] font-bold text-white`}>
                      {['SL', 'MR', 'JP', 'AC'][i]}
                    </div>
                  ))}
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.5 }} className="mt-6 flex justify-end">
                <button onClick={() => handleScroll('#approach')} className="h-14 w-14 rounded-full bg-gray-900 text-white flex items-center justify-center hover:bg-pw-600 transition-colors duration-300 hover:scale-105 active:scale-95 shadow-lg">
          ...
          {/* Block 3 — Image with text overlay right */}
          <AnimatedSection className="md:col-span-3 md:row-span-2">
            <div className="rounded-bento relative h-full min-h-[340px] group cursor-pointer overflow-hidden">
          ...
          <AnimatedSection className="md:col-span-2" delay={0.05}>
            <div className="rounded-bento relative min-h-[200px] group cursor-pointer overflow-hidden">
              <img src={IMAGES.workspace} alt="" className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
